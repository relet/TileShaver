...
if __name__ == '__main__':
    ...
    if len(sys.argv)>1:
        mini = json_minify(open(sys.argv[1],"r").read())
        if len(sys.argv)>2 and sys.argv[2] == "int":  # strip all floats to integers, useful for UTM coordinates
          re_digits = re.compile('(\d+)\.\d+')
          print re_digits.sub('\\1',mini)
        else:
          print mini
        sys.exit(1)
